<template>
  <div v-show="showMusic">
    <transition  name="Down">
      <div class="music" ref="music" v-show="musicView">
      <div class="background">
        <img :src="datas.al.picUrl" alt="">
      </div>
      <div class="top">
        <i @click="back" class="back icon iconfont icon-moreunfold1"></i>
        <h1 class="title">{{datas.name}}</h1>
        <h2 class="username">{{datas.ar.name}}</h2>
      </div>
       <transition  name="fade">
          <div @click="currentview" v-show="currentView" class="middle">
        <div class="middle-l">
      ...
      <transition  name="fade">
      <div  @click="currentview"  v-show="!currentView" class="lyric" ref="lyricList">
        <ul ref="lyricWrap">
          <li v-for="(item,index) in lyric" v-html="item.s?item.s:'～～～'" v-if="lyric" :data-time="item.t" :class="index==flag?'yellow':''"></li>
        </ul>
        <div class="text" v-show="lyric.length == 0">哎呦~人家没找到</div>
      </div>
      </transition>
      <div class="bottom">
    ...
    <div class="min-player"  v-show="!musicView"  @click="showBMusic" ref="minMusic">
...
<script type="text/ecmascript-6">
  ...
  import {setLyric} from 'common/js/lyric'
  ...
  export default {
    ...
    watch: {
      source() {
        ...
        getLyric(id).then(res => {
          ...
          if(res.code == ERR_OK) {
            if(res.lrc){
              this.lyric = setLyric(res.lrc.lyric);
            }
          }
        });
    ...
    methods: {
      ...
      readyM(e) {
        this.play = true;
        //设置总时长
        this.duration = zero(e.target.duration);
      },
      endM() {
        this.play = false;
      ...
      updateTime(e) {
        ...
        this.currentTime = zero(e.target.currentTime);
  ...
  .min-player {
    ...
    .btn {
      ...
      .canvas{
        ...
        border-radius: 50%;
        z-index: 9;
        border:1px solid rgba(102, 102, 110,.6);
      }
    }
    .list {
      flex: 0 0 40px;
